...
internal sealed class RepositoryPaths
{
    ...
    public string WebDataDirectory => Path.Combine(SolutionRoot, "NEMSweep.Web", "wwwroot", "data");
    ...
    public static RepositoryPaths Discover(string startPath)
    {
        ...
        {
            if (File.Exists(Path.Combine(candidate, "NEMSweep.slnx")))
            {
                return new RepositoryPaths(candidate);
            }

            candidate = Directory.GetParent(candidate)?.FullName
                ?? throw new DirectoryNotFoundException("Could not locate the NEMSweep solution root.");
        }
