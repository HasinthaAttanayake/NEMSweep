...
{
    /// <summary>Reported when the binary carries no build commit, and no result can name a model.</summary>
    public const string UnknownCommit = "unavailable";

    /// <summary>
    /// Reads the model build a run was made from: the commit the binary was built at, and whether
    /// anything uncommitted was in play when it ran.
    /// </summary>
    /// <remarks>
    /// The commit comes from <see cref="BuildProvenance"/>, not from the directory the command was
    /// launched in. Uncommitted changes are only asked about when that directory is a checkout
    /// standing on the same commit, because that is the only case where the working tree in front of
    /// git is the source this binary was built from.
    /// </remarks>
    ...
    {
        string? buildCommitSha = BuildProvenance.CommitSha;
        if (buildCommitSha is null)
        {
            return new SweepRunMetadata(UnknownCommit, false);
        }

        bool runFromTheSourceTree = TryRunGit(workingRoot, "rev-parse", "HEAD") == buildCommitSha;
        string? status = runFromTheSourceTree
            ? TryRunGit(workingRoot, "status", "--porcelain")
            : null;
