...
using ContractsScenario = NEM.Contracts.Scenario;
...
public sealed class ScenarioRunnerMultiRegionTests
{
    ...
    private sealed class RunnerFixture : IDisposable
    {
        ...
        {
            RootPath = Path.Combine(Path.GetTempPath(), $"nemsim-runner-{Guid.NewGuid():N}");
            OutputRoot = Path.Combine(RootPath, "output");
            Directory.CreateDirectory(OutputRoot);
            Directory.CreateDirectory(Path.Combine(RootPath, "NEM.CLI"));
            File.WriteAllText(Path.Combine(RootPath, "NemSim.slnx"), string.Empty);
            File.WriteAllText(
                Path.Combine(RootPath, "NEM.CLI", "appsettings.local.json"),
