...
public sealed class DispatchRunContextTests
{
    ...
    [Fact]
    public void ConfigRepositoryUrl_UsesTheWebProjectFolderTheSweepWasActuallyBuiltAgainst()
    {
        SweepIndexDTO index = Index(Point("p0")) with
        {
            Provenance = ArtifactFixtures.Index(Point("p0")).Provenance with
            {
                InputFiles =
                [
                    new SweepInputFileDTO("NEM.Web/wwwroot/data/demand-nsw1.json", "demand-data", "hash"),
                ],
            },
        };
        DispatchRunContext context = DispatchRunContextResolver.Resolve(index, "test-sweep", "p0")!;

        context.ConfigRepositoryUrl.Should().Contain("/NEM.Web/wwwroot/");
    }
