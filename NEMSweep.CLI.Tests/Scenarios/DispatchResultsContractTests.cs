...
public sealed class DispatchResultsContractTests
{
    ...
    public void Export_PublishesCanonicalPerFleetDeliveredGeneration(bool includesStorage)
    {
        ...
        string rootPath = Path.Combine(Path.GetTempPath(), $"nemsweep-dispatch-export-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            if (includesStorage)
            {
                result.PowerSystem.StorageFleets.Should().ContainSingle().Which
                    .Should().Be(new DispatchStorageFleetDTO(
                        "Battery", 120, 30, TechnicalLifeYears: 15, RoundTripEfficiency: 0.87));
