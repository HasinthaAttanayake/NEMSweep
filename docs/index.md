...
NemSim is a system-planning model, not a market simulator and not a forecast.
...
NemSim is being built in public, one validated layer at a time. The
[results site](https://nemsim.pages.dev/) shows the current state of that work, and the
[repository](https://github.com/HasinthaAttanayake/NemSim) carries the model, the CLI that produces
