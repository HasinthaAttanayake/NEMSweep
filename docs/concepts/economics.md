...
A scenario models a single year, but the assets in it — a coal plant, a battery, an interconnector —
last decades. `LevelisedCostCalculator` spreads a one-off capital cost across an asset's technical
...
gives the equal annual payment that would repay it, at that rate, over that life — which is how a
single modelled year can carry its fair share of an asset that will still be running in year twenty.
...
introduced, not just what the scenario originally declared. There is no separate charge for the
...
against the same served-energy denominator as everything else, not the delivered cost of energy that
passed through storage.

## The denominator

Every levelised figure — generation, storage, transmission, and the total — divides by
...
region. Every way of doing that split — by rating, by realised flow, evenly — is arbitrary, so the
model does not attempt one. The consequence is a genuine one, not an oversight: summing every region's
...
Measured physical quantities — power, energy, distance — remain `double`. The two only meet inside
typed conversion methods (`PowerCapacityCost.For(Power)`, `EnergyPrice.For(Energy)`, and so on), which
