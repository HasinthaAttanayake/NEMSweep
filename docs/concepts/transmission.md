...
link belongs to neither region alone — attributing it to one side or the other would be arbitrary. A
corridor that carries flow both ways is not one bidirectional link; it is **two** directed
interconnectors, each independently declared, each independently costed. That convention matters for
economics too — see [Economics](economics.md) and
...
That guarantee — a higher-priority region is never starved by a lower-priority one — is also exactly
why the result is not a global optimum. A different allocation might serve total demand better in
aggregate while starving the top-priority sink a little; the solver will never choose that allocation.

## Why a sink is solved iteratively

A sink's requirement is stated at the **receiving** end — how much it still needs delivered — but
...
decomposed into paths after the solve. Those two units don't reconcile in one shot, so each sink is
...
energy can never exceed sent energy, each round can only reduce the outstanding requirement, never
overshoot it — so the sequence converges geometrically toward exactly satisfying the sink, or toward
the network's true capacity limit if it cannot be fully satisfied.
...
An export draws first on generation that would otherwise be curtailed — moving that energy to an
export costs nothing extra and starts no new plant — and only then on dispatchable headroom, started
specifically to serve the export, in ascending merit order. Pumped hydro is excluded from what can be
exported entirely, because storage is decided *after* transfer runs for the interval; by the time
storage would have something to offer, the export decision has already been made.
...
decomposition — two independent derivations of the same quantity, reconciled rather than assumed
consistent.
...
*solar* weather sites — the only source of a region's location in the model — and its directed
transfer capacity, using the `DistancePowerCost` and `AnnualDistancePowerCost` rates described in
[Economics](economics.md). Two approximations compound in that route length, and both inflate
reported transmission cost: the solar site is chosen for resource quality, not for where a real
transmission line terminates, so modelled distance runs long against the actual NEM corridors; and
because each corridor is declared as two directed interconnectors, the same kilometre of conductor is
paid for twice, once per direction. See
[Limitations §6](../assumptions/limitations.md#6-transmission-route-length-is-a-proxy-and-it-runs-long)
for the full model-versus-actual distance table and the roughly 1.8× cost consequence of double-costing
reciprocal links — it is not repeated here.
