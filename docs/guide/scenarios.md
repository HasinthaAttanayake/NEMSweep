...
enforces the same thing at the JSON level (`UnmappedMemberHandling.Disallow`). An unknown property
anywhere in the file — a typo in a field name, a field copied from the wrong nesting level — is a
hard error, not a value that gets silently ignored. That is deliberate: a scenario that loaded
despite a typo would run with different assumptions than the author intended, and the model would
never say so.
...
|---|---|---|---|---|
| `schemaVersion` | integer | yes | — | Must equal the version reported by `--describe-schema scenario`. |
| `id` | string | yes | — | Scenario identifier. Becomes part of published artifact paths. |
| `name` | string | yes | — | Human-readable name. |
| `costBasis` | object | yes | — | See [`costBasis`](#costbasis). |
| `regions` | array of [region](#regions) | yes, at least one | — | One entry per NEM region the scenario dispatches. |
| `storageSizing` | object | yes | — | See [`storageSizing`](#storagesizing). |
| `interconnectors` | array of [interconnector](#interconnectors) | no | — | Directed transmission links between regions. |
| `provenance` | object | no | — | Free-form; the CLI overwrites this on sweep-generated configs, so treat it as informational rather than as configuration. |
...
| Field | Type | Required | Unit | Meaning |
|---|---|---|---|---|
| `regionId` | string | yes | — | One of the five NEM regions: `NSW1`, `QLD1`, `SA1`, `TAS1`, `VIC1`. Must be distinct across `regions`. |
| `demandFile` | string | yes | — | Path to this region's demand series (as written by `--ingest`). |
| `weatherFile` | string | yes | — | Path to this region's weather series (as written by `--ingest`). |
| `generatingFleets` | array of [generating fleet](#regionsgeneratingfleets) | yes, at least one | — | This region's generation. Technologies must be distinct within the region. |
| `storageFleets` | array of [storage fleet](#regionsstoragefleets) | yes, at least one | — | This region's storage. Technologies must be distinct within the region. |
...
| Field | Type | Required | Unit | Meaning |
|---|---|---|---|---|
| `technology` | string | yes | — | Generation technology name (e.g. `Coal`, `Gas`, `Solar`, `Wind`, `Hydro`). Must be distinct within the region. |
| `nameplateCapacityMw` | number | yes | MW | Installed nameplate capacity. |
| `costParameters` | object | yes | — | See [cost parameters](#generation-cost-parameters). |
| `technologyProfile` | object | yes | — | See [generation technology profile](#generation-technology-profile). |
| `monthlyCapacityFactors` | array of [monthly capacity factor](#monthlycapacityfactors) | no | — | An energy budget per calendar month, used by Hydro (see below). |
...
the mechanism that lets a scenario represent Hydro as a rationed, dispatchable-but-limited resource
rather than as unlimited firm capacity.

## `regions[].storageFleets[]`

| Field | Type | Required | Unit | Meaning |
|---|---|---|---|---|
| `technology` | string | yes | — | Storage technology name (e.g. `Battery`, `PumpedHydro`). Must be distinct within the region. |
...
| `costParameters` | object | yes | — | See [storage cost parameters](#storage-cost-parameters). |
| `technologyProfile` | object | yes | — | See [storage technology profile](#storage-technology-profile). |

`initialEnergyCapacityMwh` and `initialPowerCapacityMw` must **either both be zero or both be
positive** — a half-built fleet is rejected. A zero/zero fleet is not the same as omitting the
technology: it means no capacity of that technology is installed at the start of the run, but the
...
| `reliabilityStandardName` | string or null | no | — | Free-form label recorded against the run's reliability result. Does not affect dispatch. |
...
by how much it remained short — "no Battery of this size meets the standard" is itself a published
...
| Field | Type | Required | Unit | Meaning |
|---|---|---|---|---|
| `fromRegionId` | string | yes | — | Sending-end region. Must be one of `regions`. |
| `toRegionId` | string | yes | — | Receiving-end region. Must be one of `regions` and different from `fromRegionId`. |
...
`fromRegionId`/`toRegionId` pair each direction — and each entry is costed independently at the
