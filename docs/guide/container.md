...
matches and only `runId` differs, `provenance` included. The image reports the commit it was built
at because that commit is stamped into the binary at build time, so an image built from a clone and
a run from that clone name the same model.
