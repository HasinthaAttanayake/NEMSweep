...
1. `appsettings.local.json` — machine-local, gitignored, and absent by default.
...
| `outputRoot` | Where ingestion and scenario runs write published artifacts — normally `NEM.Web/wwwroot/data`, the directory the web site reads from. |
| `defaultScenarioPath` | The scenario configuration `--run-scenario` uses when you do not pass one explicitly. |

All three are plain strings resolved relative to the solution root at run time — see the
[CLI reference](cli.md) for how command-line paths are resolved the same way — so you can point
...
With no argument, this loads the scenario at `defaultScenarioPath` — the committed
...
(`demand-{region}.json`, `weather-{region}.json` under `outputRoot`) and dispatches every hour of
the modelled year, sizing storage against the scenario's reliability standard as it goes.

It writes `results.json`, `results-overview.json`, and a `results-{region}.json` /
`results-{region}-overview.json` pair for each region, all under `outputRoot`. Publication is
atomic, so a failed run never leaves a half-written artifact in place.
...
`NEM.Web/wwwroot/data` — the same files `--run-scenario` and `--ingest` write. Open the URL it
prints to browse the results you just generated.
