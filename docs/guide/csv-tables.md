...
Join facts to dimensions on `hourIndex`, `regionId`, `technology`, `scalarName` and `pointId`.
...
spreadsheet and read directly. `dim_scalar` carries the label and unit for each column.
